...
export function ChatMessage({ content, role, timestamp }: ChatMessageProps) {
  return (
    <div className={cn(
      ...
      role === 'assistant' ? 'bg-muted/50 dark:bg-gray-800' : 'bg-background dark:bg-gray-900'
    )}>
      ...
      <div className="space-y-1">
        <div className="flex items-center">
          <p className="text-sm font-medium leading-none text-foreground dark:text-gray-200">{role === 'user' ? 'You' : 'Assistant'}</p>
          <p className="text-xs text-muted-foreground dark:text-gray-400 ml-2">{timestamp}</p>
        </div>
